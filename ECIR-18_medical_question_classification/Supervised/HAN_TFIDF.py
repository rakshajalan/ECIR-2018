...
#model1.fit(x_train, y_train,batch_size=32,epochs=10,verbose=1,validation_data=(x_val, y_val))
...
model2 = Sequential()
model2.add(Dense(64,batch_input_shape=(None,30000)))
...
final_model.fit([x_train,input_list1], y_train,batch_size=32,epochs=12,verbose=1,validation_data=([x_val,test_data], y_val))
score = final_model.evaluate([x_val,test_data],y_val,batch_size=32,verbose=0)
y_pred=final_model.predict_classes([x_val,test_data])
f2=open("/home/jalan.raksha/2017_research/HAN_result_12.txt","w")
for ji in range(len(x_val)):
 if y_pred[ji] != Y_test[ji]:
    f2.write(str(text_x[ji])+"Predicted:"+str(y_pred[ji])+"--Actual:"+str(Y_test[ji])+"\n")

p=confusion_matrix(Y_test, y_pred)

print p
print"wide and deep_han_with dropout"
